...
main! = |_args|
    ...
    # Run on nonexistent directory - should return empty results (not error)
    results = Spec.run!("tests/this_directory_does_not_exist_12345", config)?

    if List.is_empty(results) then
        Stdout.line!("PASS: Nonexistent directory returns empty results list (no error)")
    else
